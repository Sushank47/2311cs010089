...
# Stage 6: Priority Inbox Implementation & Full-Stack Baseline HTTP Integration

This section explains the full-stack baseline implementation, the Priority Inbox sorting comparator, the automated JWT token caching and renewal policies, and the logger middleware integration.

## 1. Priority Inbox Sorting Logic

Our backend retrieves notification items directly from the protected evaluation endpoint and applies a multi-criteria comparator to establish sorting order:
1. **Category Weight:** `Placement` (Weight 3) > `Result` (Weight 2) > `Event` (Weight 1).
2. **Recency:** If notifications have equal priority weights, they are sorted by creation date (`Timestamp`) in descending order (newest first).
...
## 2. Automated Token Caching & Proactive Renewal

Since the evaluation notifications API is protected by JWT Bearer tokens that expire, our backend features an autonomous `tokenManager` service:
- On startup, it calls the credentials endpoint `/evaluation-service/auth` using the student's unique Client ID and Client Secret.
- It parses the absolute expiration epoch from the `expires_in` response field.
- The token is cached in memory. If any API endpoint receives a request, the service inspects the token lifespan. If the token is within 5 minutes of expiration, it automatically and transparently renews it in the background, ensuring 100% API availability.

---

## 3. Compliant Logging Middleware

Our implementation connects directly to the server's logging facility:
1. **Package Mappings:** Translates local directory package names to the strict singular labels required by the REST log endpoint:
   - `web-backend-controllers` -> `controller`
   - `web-backend-routes` -> `route`
   - `web-backend-middleware` -> `middleware`
   - `web-backend-utils` -> `utils`
   - `web-frontend-components` -> `component`
   - `web-frontend-pages` -> `page`
   - `web-frontend-hooks` -> `hook`
2. **Character Count Constraints:** The evaluation API enforces a strict limit of 48 characters for log messages. The logger middleware automatically truncates all outgoing payloads (`message.substring(0, 48)`) before transmission, ensuring log uploads never fail.

---

## 4. Efficient Top-N Maintenance

To maintain the top 10 list efficiently as new notifications arrive in real time, re-sorting a growing collection of $K$ notifications on each insert is highly inefficient ($O(K \log K)$). 

### Highly Scalable Solution: Min-Heap / Min-Priority Queue
We use a **Min-Heap** of a fixed size $N = 10$:
- The root of the Min-Heap always represents the minimum priority element currently in the top 10 list.
- **On insertion of a new notification:**
  - If the heap has fewer than 10 elements, push it onto the heap ($O(\log 10)$).
  - If the heap already has 10 elements, compare the new notification's priority with the root element.
  - If the new notification has a higher priority, remove the root element and insert the new notification ($O(\log 10) \approx O(1)$).
  - Otherwise, discard the new notification (it is not in the top 10).

---
---

# Stage 7: Real-Time Event Integration (Socket.io)

This section outlines how live notifications are pushed from the backend to browser dashboards using **WebSockets** (via Socket.io).

## 1. WebSockets Event Architecture
To establish instant real-time synchronization, the backend initializes a WebSockets connection event listener bound to the HTTP service layer:
- **Port:** Port 5001 is mapped as the central entry point for HTTP APIs and WebSockets connections.
- **Bi-directional Pipeline:** Clients establish connections using `socket.io-client`. On connection, the client connects to the WebSocket stream.

```
  ┌──────────────┐         1. POST /api/notifications       ┌─────────────────┐
  │  Client/Admin│─────────────────────────────────────────>│  Express Route  │
  └──────────────┘                                          └────────┬────────┘
                                                                     │
                                                           2. Broadcast Socket
                                                                     ▼
  ┌──────────────┐       3. Push event "new-notification"   ┌────────┴────────┐
  │  Connected   │<─────────────────────────────────────────┤  Socket.io      │
  │  Dashboards  │                                          │  Server Engine  │
  └──────────────┘                                          └─────────────────┘
```

## 2. Broadcast Payload Schema
When the administrator publishes a notification using `POST /api/notifications`, the server broadcasts a `new-notification` event containing the structured item details:

```json
{
  "_id": "local-abc123xyz",
  "title": "Placement Update: Direct Hiring Google",
  "message": "Direct link sent to qualified CS students.",
  "type": "Placement",
  "priority": "Critical",
  "isRead": false,
  "createdAt": "2026-07-01T12:28:00.000Z"
}
```

This immediately triggers active client-side hooks to prepend the announcement in memory, recalculate the unread counter badge, and play ambient alerts.
